...
router.get("/admin/booking/:id", verifyTokenAdmin, async (req, res) => {
  ...
  let allBookings = await Booking.find({})
    ...
    .populate("serviceId")
    .sort({ _id: -1 });
  let userBookings = await allBookings.filter(
    (data) => data.userId.email === bookingid
  );
  if (userBookings.length <= 0)
    res.status(400).send("there is no booking for this email address");
  else {
...
router.get("/info", verifyToken, async (req, res) => {
  ...
  let bookings = await Booking.find({ userId: reqUser._id })
    .populate("serviceId")
    .sort({ _id: -1 });
  res.send(bookings);
});
...
router.put("/update", verifyToken, async (req, res) => {
  ...
  await Booking.findByIdAndUpdate(
    bookingid.id,
    {
      $set: { bookingStatus: "Canceled", rejectReason: bookingid.rejectReason },
      upsert: true,
    },
    { new: true },
    (err, result) => {
  ...
  if (userData) {
    ...
    if (reqUser) {
      ...
      if (service) {
        ...
        if (resBookings) {
          let bookinfInfo = await Booking.findById(resBookings._id)
            .populate("userId")
            .populate("serviceId");
          sendMail(
            bookinfInfo,

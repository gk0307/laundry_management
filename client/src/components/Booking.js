...
class Booking extends Component {
  ...
  admin = () => {
    api
      .get("/admin/enable", getClientRequestConfig())
      .then(async (Response) => {
  ...
  fetchServicesUnits = () => {
    api
      .get("/serviceUnits/user", getClientRequestConfig())
      .then(async (Response) => {
        console.log(Response.data);
  ...
  fetchServices = () => {
    api
      .get("/services/user/service", getClientRequestConfig())
      .then(async (Response) => {
        console.log(Response.data);
  ...
  render() {
    const Services = ({ data }) => {
      return (
        <div
          ...
          className={
            this.state.servicename.name === data.serviceName
              ? "serviceboxActive"
              : "servicebox"
          }
          id="hello"
          ...
          <div
            className={
              this.state.effect.length <= 0
                ? "article-flex"
                : design(data.unitName)
            }
            ref={this.myCard}
            onClick={(e) => addCart(data, index)}
          >
            <img
              src={`${dataApi}/serviceUnits/${data._id}/image`}
              alt="error"
            />
            <span>{data.unitName}</span>
          </div>
          <span
            className="span-qty-div"
            className={
              this.state.effect.length <= 0
                ? "spanNone"
                : spandesign(data.unitName)
            }
          >
            <span
              onClick={() => addArticles(index, "increment", data.unitName)}
            >
              +
            </span>
            <span>{data.qty}</span>
            <span
              onClick={() => addArticles(index, "decrement", data.unitName)}
            >
              -
            </span>
          </span>
    ...
    const addCart = async (data, index) => {
      if (this.state.effect.map((unit) => unit).includes(data.unitName)) {
        await this.setState({
          effect: this.state.effect.filter((unit) => unit !== data.unitName),
        });
      } else {
      ...
      if (datas.map((like) => like.unitName).includes(data.unitName)) {
        await this.setState({
          cartItems: this.state.cartItems.filter(
            (item) => item.unitName !== data.unitName
          ),
        });
    ...
    const checkTotal = () => {
      if (
        this.state.cartItems.reduce(
          (sum, i) => (sum += i.unitQty * i.unitRate),
          0
        ) === 0 &&
    ...
    const Handlebooking = async (event) => {
      ...
      const Booking = {
        ...
        specialNotes: this.props.users.notes,
        bookingUnits: this.state.cartItems,
        payments: {
          paymentMethod: "pay Local",
          amount: await this.state.cartItems.reduce(
            (sum, i) => (sum += i.unitQty * i.unitRate),
            0
          ),
      ...
      api
        .post("/booking", Booking, getClientRequestConfig())
        .then((Response) => {
    ...
    const CartItems = ({ items }) => {
      return (
        <div className="items-purchased order-layout">
          <div id="item">
            <img
              src={`${dataApi}/serviceUnits/${items.unitImg}/image`}
              height="30px"
              width="30px"
              alt="item pic"
            />
          ...
          <Modal>
            <div>
              <img
                ...
                height="auto"
                width="100%"
                alt="eroor on booking images"
              />
              <div className="choose-heading">
                <h3>
                  {" "}
                  <div> Service Timing 9AM - 5PM </div> Bookings are didnt
                  accecpt by this time sorry try after some time
                </h3>
              </div>
            </div>
          </Modal>
              ...
              <div className="booking-service">
                <form onSubmit={Handlebooking}>
                  <div className="booking-service-heading">
                    <div className="choose-heading">
                      {" "}
                      <h3>where would you likeus to provide service?</h3>
                    </div>
                    <div className="booking-service-input">
                      ...
                      {this.state.zipcode === "" ? (
                        <div style={{ color: "red" }}>
                          please fill Out with 6 digit
                        </div>
                      ) : (
                        <div style={{ color: "green" }}> valided pincode</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="choose-service">
                    ...
                    <div className="choose-service-container">
                      {this.props.services.map((dataItem, index) => (
                                ...
                                <div>
                                  ...
                                  <div className="items-purchased order-layout">
                                    {this.state.servicename.name && (
                                      <div id="item">service</div>
                                    )}
                                    {this.state.pickupDate &&
                                    this.state.deliveryDate ? (
                                      <div id="qty">Pickup Date</div>
                                    ) : (
                                      false
                                    )}
                                    {this.state.deliveryDate &&
                                    this.state.pickupDate ? (
                                      <div id="price"> delivery Date</div>
                                    ) : (
                                      false
                                    )}
                                  </div>
                                  ...
                                  <div className="items-purchased order-layout">
                                    <div id="item">
                                      {" "}
                                      <img
                                        src={`${dataApi}/services/${this.state.servicename.imgurl}/image`}
                                        height="30px"
                                        width="30px"
                                        alt="service name error"
                                      />{" "}
                                      {this.state.servicename.name}
                                    </div>
                                    {this.state.deliveryDate && (
                                  ...
                                  <div className="order-summary-header order-layout">
                                    ...
                                    <div id="qty">
                                      {this.props.users.fname +
                                        "" +
                                        this.props.users.lname}
                                    </div>
                                  </div>
                                  <div className="order-summary-header order-layout">
                                    <div id="item">Address</div>
                                    <div id="qty">
                                      {this.props.users.address +
                                        "," +
                                  ...
                                  <div className="order-summary-header order-layout">
                                    ...
                                    <div id="qty">{this.props.usersode}</div>
                                  </div>
                                  <div className="order-summary-header order-layout">
                                    <div id="item">Phone number</div>
                                    <div id="qty">
                                      {this.props.users.phoneNo}
                                    </div>
                                  </div>
                                  <div className="order-summary-header order-layout">
                                    <div id="item">email</div>
                                    <div id="qty">{this.props.users.email}</div>
                                  </div>
                                  <hr />
                                  <div className="order-summary-header order-layout">
                                    <div id="item">Notes</div>
                                    <div id="qty">{this.props.users.notes}</div>
                                  </div>
                                </div>
                              )}
                            </div>
...
const mapStateToProps = (state) => ({
  ...
  //SERVICE
  services: state.entities.service.list,
});
const mapDispatchToProps = (dispatch) => ({
  loaduser: () => dispatch(loadUser()),
  updateuser: (data) => dispatch(updateUser(data)),
  //services
  getService: () => dispatch(loadservice()),
});
